...
def run_test(token):
    # إعداد قاعدة البيانات داخل الدالة
    conn = sqlite3.connect('league_test.db', check_same_thread=False)
    cursor = conn.cursor()
    ...
    # تعريف المتغيرات لتكون متاحة داخل الدوال
    global cur_local
    cur_local = cursor
    global conn_local
    conn_local = conn

    # --- منطق الأوامر (تم نقلها هنا) ---
    COUNT, TEAM = range(2)
    ...
    async def laws_cmd(u, c):
        cur_local.execute("SELECT content FROM laws"); res = cur_local.fetchone()
    ...
    async def add_laws_cmd(u, c):
        ...
        cur_local.execute("DELETE FROM laws"); cur_local.execute("INSERT INTO laws (content) VALUES (?)", (new_laws,)); conn_local.commit()
        await u.message.reply_text("✅ تم تحديث القوانين.", parse_mode='Markdown')

    async def setup_entry(u, c):
        if not await is_admin(u): return
        cur_local.execute("SELECT count(*) FROM matches")
    ...
    async def team_h(u, c):
        ...
        cur_local.execute("DELETE FROM teams"); cur_local.execute("DELETE FROM matches")
        for t in teams: cur_local.execute("INSERT INTO teams VALUES (?)", (t,))
        ...
        for i, r_matches in enumerate(rounds):
            for m in r_matches: cur_local.execute("INSERT INTO matches (team1, team2, round_num) VALUES (?,?,?)", (m[0], m[1], i+1))
        conn_local.commit(); await u.message.reply_text("✅ تم إنشاء الدوري!"); return ConversationHandler.END

    async def table_cmd(u, c):
        cur_local.execute("SELECT name FROM teams"); teams = [r[0] for r in cur_local.fetchall()]
        if not teams: await u.message.reply_text("الدوري فارغ."); return
        stats = {t: {'j':0, 'g':0, 'n':0, 'p':0, 'b':0, 'pts':0} for t in teams}
        cur_local.execute("SELECT team1, team2, s1, s2 FROM matches WHERE played=1")
        for t1, t2, s1, s2 in cur_local.fetchall():
    ...
    async def round_cmd(u, c):
        cur_local.execute("SELECT MIN(round_num) FROM matches WHERE played=0"); res = cur_local.fetchone()
        if not res or res[0] is None: await u.message.reply_text("انتهى الدوري!"); return
        cur_local.execute("SELECT team1, team2 FROM matches WHERE round_num=? AND played=0", (res[0],))
        await u.message.reply_photo(photo=create_fixture_image(res[0], cur_local.fetchall()))
    ...
    async def btn_h(u, c):
        ...
        if data == 'ok':
            ...
            cur_local.execute("UPDATE matches SET s1=?, s2=?, played=1 WHERE team1=? AND team2=? AND played=0", (m['s1'], m['s2'], m['t1'], m['t2']))
            conn_local.commit(); await q.edit_message_text("✅ تم الحفظ!")
        elif data == 'no': await q.edit_message_text("🚫 ألغيت.")
        elif data == 'confirm_setup':
            cur_local.execute("DELETE FROM matches"); cur_local.execute("DELETE FROM teams"); conn_local.commit()
